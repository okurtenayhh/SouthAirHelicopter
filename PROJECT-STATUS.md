...
- **The site launches in five phases, not all at once** — decided 2026-09-08; the full plan and its gates are in `docs/launch-plan.md`. Phase 1 is Home, Services and Contact plus the domain switch; later phases add pages as their content is confirmed. The mechanism is a `LAUNCHED` list in `tools/verify.py` that both makes a page public and flips its placeholder rule from "3 or more" to "exactly zero", so the build refuses to ship a live page with unfinished copy.
