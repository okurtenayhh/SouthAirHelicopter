...
**Coming-soon landing page (one page, built for the real domain): https://sah-coming-soon.netlify.app** — **GO-LIVE DONE 2026-08-10: indexable, verified by real HTTP request returning no `X-Robots-Tag` at all. Only DNS remains — see Next Up item 1.** **not attached to the client's domain — no DNS change has been made, and the page is not public and not indexed.** Its own Netlify site, separate from the preview above (which is untouched): site id `de01967d-071f-433e-a5af-6e87b7870b22`, site name `sah-coming-soon`.
...
1. **Point the domain at the coming-soon page. Only the Squarespace DNS edit is left.** **The domain is `southairhelicopters.com`** — registered at Squarespace, active, expires 2029-07-31. *(This string was missing from this file for five sessions. It is now recorded here and nowhere else — keep it.)* Done already on 2026-08-10: the `[[headers]]` block deleted and redeployed, go-live verified by real HTTP request returning **no `X-Robots-Tag` at all**; and the domain attached to the Netlify site as `custom_domain` with `www.southairhelicopters.com` as an alias.
   - **In Squarespace, delete the whole "Squarespace Defaults" preset** (trash icon on that panel). It holds four `A` records on `@` pointing at Squarespace's own IPs (`198.49.23.144/145`, `198.185.159.144/145`), a `CNAME` on `www` to `ext-sq.squarespace.com`, and an `HTTPS` record whose `ipv4hint` also names the Squarespace IPs. **Leaving any of these means the domain keeps resolving to Squarespace** — the apex currently still does.
   - **Then add two custom records:** `A` / host `@` / value **`75.2.60.5`** (Netlify's apex load balancer, resolved from `apex-loadbalancer.netlify.com`), and `CNAME` / host `www` / value **`sah-coming-soon.netlify.app`**.
   - **Leave the `_domainconnect` CNAME and the Email Security TXT records alone** — they are not part of website routing.
   - **Then wait for propagation and confirm** the domain serves the page over HTTPS and Netlify has issued the certificate. Netlify cannot issue it until the records resolve to Netlify.
   - The `.netlify.app` URL is indexable too now. Expected and harmless; once the real domain serves, that is the canonical one.
