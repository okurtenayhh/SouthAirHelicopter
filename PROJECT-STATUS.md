...
4. **Turn the chosen logo into usable artwork.** The design is settled (`south-air-5a.jpg`, see Decisions Locked); what exists is a flat 1160×822 JPEG with an opaque `#f2f2f0` ground, which cannot be used anywhere on the site — every logo slot is light-on-navy, and the favicon needs to work at 32px. **Get the vector source from whatever produced the set** before doing anything else; that single answer also settles the provenance question below. Then the deliverables are: a horizontal lockup, an icon-only mark, a favicon, light and dark variants, and a reduced high-contrast version that can be stitched. **The single highest-value move here is to re-run the aircraft generation against the rendering constraints the 2026-08-06 prompt already specified and the tool ignored** — flat tones, solid fills, readable as a solid-black silhouette. That one action produces the icon mark, the favicon and the embroiderable version at once, and it needs nobody's permission. See Constraints That Bite for the prompt's location and the one uncertainty to fix in it. **What still has to be answered before the mark ships publicly:** whether Bell is content with an identifiable **429** inside South Air's own mark (on the list for Zachariah at item 3, and now the sharpest question on it), plus a confirmation from the user of the generation story recorded under Constraints. **Note the rest of the mark is reconstructable in vector today** — the wordmark, band, rules and layout are type and rectangles. The aircraft is the only blocked element.
...
- **The aircraft in the logo is a Bell 429, deliberately, and its provenance is traced** (recovered from `.recall/history.md` on 2026-08-10 — recording it here so it is never re-derived). It was generated by an AI image tool from a prompt written in the 2026-08-06 session. That prompt's proportions and feature list came from two cross-validated sources: Bell's own 92-page 429 specification PDF, and a general arrangement drawing from **the-blueprints.com**, which carries a watermark and is a **licensed commercial product**. **Two exposures, and they are narrower than "someone else's drawing" but not zero.** *(a)* The prompt took **dimensions and described features** from the blueprint, not a trace of it — measurements and factual proportions are not themselves protectable, so this is meaningfully weaker than copying the drawing. It is still the kind of thing to have a clear answer on before the mark goes on a **public commercial site** and onto **merchandise**, and the 2026-08-06 session flagged it at the time and never got a resolution. *(b)* It is an identifiable **429**, which is the exact model `tools/verify.py:23` blocks from the markup — the guard is textual so an image slips past it, but the site now *depicts* a rating the shop has not confirmed and Bell has not been asked about. **Confirm the generation story with the user, then treat (b) as the live question, not (a).**
- **The delivered aircraft ignores the rendering constraints it was asked for, and that is why it will not embroider** (2026-08-10). The 2026-08-06 prompt specified, verbatim: *flat plain single-color background*, *two or three flat tones maximum*, *solid fills and clean edges, not photographic gradients*, and **"must read as one clean silhouette — recognizable if filled solid black."** What came back is the opposite: continuous grayscale pencil shading, hairline blades, and skids-down on the ground rather than the specified in-flight attitude. **So the fix is cheap and does not require redesigning anything** — re-run the generation and hold the output to the rendering block that was already written. That single action resolves the embroidery version and the silhouette/favicon problem together. The prompt is preserved in `.recall/history.md`; **lift it from there rather than rewriting it**, and fix the one flagged uncertainty in it (it asserts a port-side tail rotor as an unverified inference — any clear photo of a 429 tail settles it).
...
**The provenance question was answered mid-session from `.recall/history.md`, not by the
user.** The aircraft is a Bell 429 generated by an AI image tool from a prompt this project
wrote on 2026-08-06, off Bell's own spec PDF plus a licensed the-blueprints.com drawing.
That is written up under Constraints That Bite and should be confirmed rather than
re-investigated. It also produced the session's most useful finding: **the generator
