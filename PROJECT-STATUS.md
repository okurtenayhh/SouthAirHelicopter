...
1. **Point the domain at the coming-soon page. Everything on this side is done — only DNS is left.** The `[[headers]]` block was deleted and the page redeployed on **2026-08-10**, and the go-live check passed: a real HTTP request returns **no `X-Robots-Tag` header at all**, which is the only correct outcome and proves `coming-soon/netlify.toml` was the config read. **The page is live and indexable now** at https://sah-coming-soon.netlify.app. What remains, in Squarespace and the Netlify UI:
   - **Add the custom domain** to the `sah-coming-soon` site (site id `de01967d-071f-433e-a5af-6e87b7870b22`) under Domain management. Netlify then displays the exact DNS records to create — **use the values it shows rather than any written here**, since they vary by setup.
   - **Create those records at Squarespace**, which is the registrar. Typically an `A` record on the apex pointing at Netlify's load balancer and a `CNAME` on `www` pointing at `sah-coming-soon.netlify.app`.
   - **Wait for propagation, then confirm the domain serves the page over HTTPS** and that Netlify has issued the certificate. **The exact domain string is still not written down anywhere — capture it in this file when you have it in front of you.**
   - Note the `.netlify.app` URL is now indexable too. That is expected and harmless; once the real domain is serving, it is the one that matters.
