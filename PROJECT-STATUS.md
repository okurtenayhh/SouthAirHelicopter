...
4. **Turn the chosen logo into usable artwork.** The design is settled (`south-air-5a.jpg`, see Decisions Locked); what exists is a flat 1160×822 JPEG with an opaque `#f2f2f0` ground, which cannot be used anywhere on the site — every logo slot is light-on-navy, and the favicon needs to work at 32px. **Get the vector source from whatever produced the set** before doing anything else; that single answer also settles the provenance question below. Then the deliverables are: a horizontal lockup, an icon-only mark, a favicon, light and dark variants, and a reduced high-contrast version that can be stitched. **Two questions must be answered before any of this ships, and both are cheap to ask now:** *(a)* what made the helicopter illustration — if it derives from the wall-art listing supplied as a reference, it is someone else's drawing on a public commercial site and on merchandise; *(b)* whether Bell is content with a recognizable Bell airframe inside South Air's own mark, which is already on the list for Zachariah at item 3. **Note the mark is mostly reconstructable in vector without either answer** — the wordmark, band, rules and layout are type and rectangles. The aircraft is the only blocked element, so it is the only thing worth waiting on.
...
- **The helicopter illustration's provenance is unknown, and this is the one that could actually bite** (2026-08-10). The reference image supplied with Mike's brief was a **wall-art product listing — someone else's copyrighted line drawing**, usable as a style reference and never as a source. The aircraft in `south-air-5a` is a detailed rendering in that same idiom, and nobody has written down what made it. Two exposures follow, both on the client rather than the user: this is going on a **public commercial website** and onto **merchandise the company intends to sell or wear**, which is the use a stock or wall-art licence is least likely to cover. **Ask what generated the set before the mark goes anywhere public.** If the answer is an AI tool or a trace of the listing, the aircraft needs redrawing from scratch — which is also the fix for the Bell-airframe question and the embroidery version, so all three collapse into one job.
...
Last session's guess that more client material was coming was right — this was it. **Two
questions were put to the user and are unanswered as of this writing:** what produced the
logo set (the provenance question), and whether anyone on the client side has approved 5a
or whether "official" is the user's own pick. Both are in Waiting On / Constraints That
Bite. **Neither blocks vectorising the wordmark, band and layout** — only the aircraft.
